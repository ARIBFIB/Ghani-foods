# step1-apply-store-schemas.ps1
# Run from: D:\Rozmarrah-CUST\Saim Ashraf\Nimko\Working\Code\GhaniFoods
# Usage: .\step1-apply-store-schemas.ps1
#
# STEP 1 of the v1.2/v2.2 gap-closure plan.
# Overwrites:
...
Write-Host "=== Step 1: Applying store.ts + schemas.ts (BRS v1.2 / Spec v2.2) ===" -ForegroundColor Cyan
...
Write-FileSmart "apps\frontend\lib\schemas.ts" $schemasContent
...
function computePackagingUnitCost(gramsPerUnit: number, rawMaterial: RawMaterial | undefined): number {
...
Write-FileSmart "apps\frontend\lib\store.ts" $storeContent

Write-Host "`n=== Step 1 complete ===" -ForegroundColor Cyan
Write-Host "Note: packaging, invoices, and payments pages will show TypeScript" -ForegroundColor Yellow
Write-Host "errors until Steps 2/3/6/7 update them to the new store shape." -ForegroundColor Yellow
Write-Host "This is expected - run 'npm run dev' just to sanity-check the store" -ForegroundColor Yellow
Write-Host "and schemas files themselves compile in isolation." -ForegroundColor Yellow