    ...
    <div className="space-y-3">
      {showGlobalFilter && (
        <input
          ...
          className="w-full max-w-sm rounded-lg border border-[var(--surface-border)] bg-[var(--surface)] px-3 py-2 text-sm text-[var(--foreground)] placeholder:text-[var(--foreground)]0 outline-none focus:border-[var(--surface-border-strong)]"
        />
      )}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {table.getRowModel().rows.length === 0 ? (
              <tr>
                <td colSpan={columns.length} className="px-4 py-8 text-center text-[var(--foreground)]0">
                  No results found.
                </td>
              </tr>
            ) : (
