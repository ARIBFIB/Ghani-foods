...
#
# What this does:
#   1. Topbar  — rewires NotificationBell to read from Zustand store
#                (so bell reacts when stock changes in-session)
#   2. Settings — wires to store (adds settings slice: businessName, address,
#                 footerText, defaultMargin, lowStockDefault) + saves on submit
#   3. Auth middleware — middleware.ts that redirects unauthenticated users
#                        to /login (uses a simple "ghanifoods-auth" cookie
#                        set on login, cleared on logout)
#   4. Login page — sets the cookie on success, clears on logout
#   5. Reports page — rewired to Zustand store + Recharts bar/line charts
#                     + client-side CSV export
#   6. TanStack Table — replaces plain <table> on Raw Materials, Packaging,
#                       Batches, Customers, Invoices, Payments with sortable
#                       + column-filterable TanStack Table v8 grids
...
# ---------------------------------------------------------------------------
# 1. lib/store.ts — add settings slice to existing store
#    We append a new store file that re-exports everything + settings.
#    Easier than patching: we rewrite the whole file with settings added.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 2. Topbar — read low-stock alerts from store (not mock-data)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 3. Login page — sets cookie on success
# ---------------------------------------------------------------------------
...
export default function LoginPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setLoading(true);
    setError("");
    // Set auth cookie (1-day expiry) — any credentials succeed (demo build)
...
# ---------------------------------------------------------------------------
# 4. middleware.ts — redirect unauthenticated users to /login
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 5. Settings page — wired to store
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 6. Shared SortableTable component (TanStack Table v8)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 7. Raw Materials list — SortableTable
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 10. Customers list — SortableTable
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 11. Invoices list — SortableTable
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 12. Payments list — SortableTable
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 13. Reports page — store wiring + Recharts + CSV export
# ---------------------------------------------------------------------------
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2 border-b border-neutral-800">
        {(["inventory", "yield", "pnl"] as const).map((key) => (
          <button key={key} onClick={() => setTab(key)}
            ...
            {key === "yield" ? "Production Yield" : key === "pnl" ? "P&L" : "Inventory Movement"}
          </button>
        ))}
      </div>
      ...
      {tab === "yield" && (
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-5">
            <h2 className="text-sm font-semibold text-neutral-200 mb-4">Output Yield, Wastage & Leftover per Batch</h2>
            <ResponsiveContainer width="100%" height={320}>
...
Write-Host "  [1] lib/store.ts        — settings slice added (updateSettings action)" -ForegroundColor Yellow
Write-Host "  [2] topbar.tsx          — NotificationBell now reads from Zustand store" -ForegroundColor Yellow
Write-Host "  [3] login/page.tsx      — sets 'ghanifoods-auth' cookie on sign-in" -ForegroundColor Yellow
Write-Host "  [4] middleware.ts       — redirects unauthenticated users to /login" -ForegroundColor Yellow
Write-Host "  [5] settings/page.tsx   — wired to store, saves with Zod validation" -ForegroundColor Yellow
Write-Host "  [6] sortable-table.tsx  — shared TanStack Table v8 component (sort + global filter)" -ForegroundColor Yellow
...
Write-Host "  [13] reports/page.tsx   — store wired + Recharts BarChart/LineChart + CSV export" -ForegroundColor Yellow
Write-Host ""
...
Write-Host "  - Topbar bell -> should show Ghee + Spice Mix as low stock" -ForegroundColor White
